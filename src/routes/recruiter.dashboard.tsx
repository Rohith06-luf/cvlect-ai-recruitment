...
import { api, type PipelineEntry, type Stats, type Activity } from "@/lib/api";
...
function RecruiterDashboard() {
  ...
  const { data: stats } = useQuery({
    queryKey: ["stats", "recruiter", 1],
    queryFn: () => api.getStats({ recruiterId: 1 }),
  });

  // Fetch pipeline candidates (paginated)
  const { data: pipelineData } = useQuery({
    queryKey: ["pipeline", 1, page, PAGE_SIZE],
    queryFn: () => api.listPipeline({ jobId: 1, page, pageSize: PAGE_SIZE }),
  });

  // Fetch total count for pagination
  const { data: pipelineCount } = useQuery({
    queryKey: ["pipeline-count", 1],
    queryFn: () => api.countPipeline({ jobId: 1 }),
  });

  // Fetch activities
  const { data: activities } = useQuery({
    queryKey: ["activities", 1],
    queryFn: () => api.listActivities(1, 20),
  });
